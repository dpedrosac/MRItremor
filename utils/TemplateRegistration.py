...
class Registration:
    ...
    def RegisterSeq(self, subjects, template_folder=os.path.join(ROOTDIR, 'data', 'template', 'icbm152')):
        ...
        for file2process, no_subj in fileIDs:
            print("\tRegistering subj: {}".format(no_subj))

            filename_template = 'mni_icbm152_t1_tal_nlin_asym_09b_hires.nii' \
                if any(re.search(r'\w+(?!_).({})|^({}[\-])\w+.|^({})[a-z\-\_0-9].'.format(z, z, z),
                                 os.path.basename(file2process), re.IGNORECASE) for z in['_MDEFT3D'] * 3) else \
                'mni_icbm152_t2_tal_nlin_asym_09b_hires.nii'

            template_imaging = ants.image_read(os.path.join(template_folder, filename_template))
            print("\n\tRegistering {} (f) to {} (m)".format(filename_template, os.path.basename(file2process)))
            output_folder = os.path.join(FILEDIR, no_subj, 'output')
            FileOperations.create_folder(output_folder)
            filename_save = os.path.join(output_folder, self.cfg['prefix'] + os.path.basename(file2process))

            if not os.path.isfile(filename_save):
                # Start with Registration to template image (fixed image)
                original_image = ants.image_read(file2process)
                registered_imaging = ants.registration(fixed=original_image, moving=template_imaging,
                                               ...
                                               initial_transform="[%s,%s,1]" % (file2process,
                ...
                ants.image_write(registered_imaging['warpedfixout'],
                                 filename=os.path.join(output_folder, 'wfi_' + os.path.basename(file2process)))
