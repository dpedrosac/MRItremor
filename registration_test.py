...
FIXED = glob.glob(os.path.join(TEST_ROOT_FOLDER, '*bcorr*.nii.gz'))
IMAGE1 = glob.glob(os.path.join(TEST_ROOT_FOLDER, '*bc_diff*.nii.gz'))

print(IMAGE1)

fixed_image = ants.image_read(FIXED[0]).resample_image((100, 100, 50), True, 0)
moving_image = ants.image_read(IMAGE1[0]).resample_image((100, 100, 50), True, 0)

# moving_image.plot(axis=2)

fixed_image.plot(overlay=moving_image, title="Before Registration", axis=0)
...
fixed_image.plot(overlay=wrapped_image, title="After Registration")
